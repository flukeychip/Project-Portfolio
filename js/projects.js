...
const projects = [
  ...
  },
  {
    ...
    description: "This is the most in-depth project I have taken on. I was inspired by the Universal Robots UR3e arms in a campus design lab, but at $20,000 they were out of my price range, so I built my own. With parts on hand, about $100 from Amazon, and an old 3D printer, I built a six-degree-of-freedom arm: 3D printed 20:1 cycloidal drives, carbon fiber struts, belt driven remote actuation, magnetic Hall effect sensors, and a Jetson running it.</p><p>It is not perfect yet. The printed drives do not hold tight enough tolerances, so the next set gets laser cut from aluminum, and the fixed I2C address on the sensors forces a multiplexer, so I am moving to parts with configurable addresses. After that comes a force sensitive gripper that commands force instead of position, using a camera to estimate what each object needs. Because it runs on the Jetson, it can also photograph what it picked up, check whether that force was right, and save a better one for next time.",
